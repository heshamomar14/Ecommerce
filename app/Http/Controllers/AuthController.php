...
class AuthController extends Controller
{
    ...
    {
        $validator = Validator::make($request->all(), ['name' => 'required|min:4', 'email' => 'required|email|unique:users', 'password' => 'required|min:4|confirmed']);
        if ($validator->passes()) {
            $user = new User();
            ...
            return response()->json(['status' => true, 'message' => 'you have been registerd successfully']);
    ...
    public function authinicate(Request $request)
    {
        ...
        if ($validator->passes()) {
            if (Auth::attempt(['email' => $request->email, 'password' => $request->password], $request->get('remember'))) {
                ...
                return redirect()->route('account.login')->withInput(request()->only('email'))->with('error', 'your email or password is incorrect.');
            }
        } else {
    ...
    {
        $userId = Auth::user()->id;
        $validator = Validator::make($request->all(), ['name' => 'required', 'email' => 'required|email|unique:users,email,' . $userId . ',id',]);
        if ($validator->passes()) {
            ...
            return response()->json(['message' => 'Your Personal Information is Updated Successfully']);
        } else {
    ...
    {
        $userId = Auth::user()->id;
        $validator = Validator::make($request->all(), ['first_name' => 'required', 'last_name' => 'required', 'email' => 'required|email', 'country_id' => 'required', 'address' => 'required', 'city' => 'required', 'zip' => 'required', 'state' => 'required',]);
        if ($validator->passes()) {
            CustomerAddress::updateOrCreate(['user_id' => $userId], ['user_id' => $userId, 'first_name' => $request->first_name, 'last_name' => $request->last_name, 'email' => $request->email, 'mobile' => $request->mobile, 'country_id' => $request->country_id, 'address' => $request->address, 'apartment' => $request->apartment, 'city' => $request->city, 'zip' => $request->zip, 'state' => $request->state,]);
            session()->flash('success', 'Your Address Information is Updated Successfully');

            return response()->json(['message' => 'Your Address Information is Updated Successfully']);
        } else {
            return response()->json(['status' => true, 'errors' => $validator->errors()]);
        }
    }
    ...
    public function orderDetails(Request $request, $id)
    {
        ...
        return view('front.account.orderDetails', ['orders' => $orders, 'orderItems' => $orderitems, 'orderItemsCount' => $orderitemsCount]);
    }
    ...
    public function ProcessResetPassword(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), ['new_password' => 'required|min:5|same:confirm_password', 'confirm_password' => 'required']);
        if ($validator->fails()) {
